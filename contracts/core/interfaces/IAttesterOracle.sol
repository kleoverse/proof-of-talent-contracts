...
/**
 * @title IAttesterOracle
 * @author Sahil Vasava
 * @notice This is the interface for the oracle attesters in Proof of Talent Protocol
 */
interface IAttesterOracle {
  event AttestationGenerated(Attestation attestation);

  event AttestationDeleted(Attestation attestation);

  event VerifyRequest(Request request, address claimer);
